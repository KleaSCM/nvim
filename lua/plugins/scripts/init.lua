...
function M.setup()
    -- Debug logging
    vim.notify("Scripts plugin setup started", vim.log.levels.INFO)
    
    -- Check if Scripts directory exists
    local scripts_dir = vim.fn.expand("~/.config/nvim/Scripts")
    if vim.fn.isdirectory(scripts_dir) == 0 then
        vim.notify("Scripts directory not found at: " .. scripts_dir, vim.log.levels.ERROR)
        -- Create the directory if it doesn't exist
        vim.fn.mkdir(scripts_dir, "p")
        vim.notify("Created Scripts directory at: " .. scripts_dir, vim.log.levels.INFO)
    else
        vim.notify("Scripts directory found at: " .. scripts_dir, vim.log.levels.INFO)
    end
    ...
    vim.notify("Scripts plugin loaded successfully", vim.log.levels.INFO)
end
